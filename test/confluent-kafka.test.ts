...
describe("instrumentation-confluent-kafka-javascript", () => {
  ...
  let messagesSent: KafkaJS.Message[] = [];

  const patchProducerSend = (cb: () => Promise<KafkaJS.RecordMetadata[]>) => {
    const origProducerFactory = KafkaJS.Kafka.prototype.producer;
    KafkaJS.Kafka.prototype.producer = function (...args): KafkaJS.Producer {
      const producer = origProducerFactory.apply(this, args);

      producer.send = function (record: KafkaJS.ProducerRecord) {
        messagesSent.push(...record.messages);
        return cb();
      };

      producer.sendBatch = function (batch: KafkaJS.ProducerBatch) {
  ...
  let runConfig: KafkaJS.ConsumerRunConfig | undefined;

  const storeRunConfig = () => {
    const origConsumerFactory = KafkaJS.Kafka.prototype.consumer;
    KafkaJS.Kafka.prototype.consumer = function (...args): KafkaJS.Consumer {
      const consumer: KafkaJS.Consumer = origConsumerFactory.apply(this, args);
      consumer.run = function (config?: KafkaJS.ConsumerRunConfig): Promise<void> {
        runConfig = config;
  ...
  describe("producer", () => {
    ...
    describe("successful send", () => {
      ...
      function initializeProducer(
        recordMetadata: KafkaJS.RecordMetadata[] = defaultRecordMetadata
      ) {
        patchProducerSend(
          async (): Promise<KafkaJS.RecordMetadata[]> => recordMetadata
        );
      ...
      it.only("simple send create span with right attributes, pass return value correctly and propagate context", async () => {
        await producer.connect()
        const res: KafkaJS.RecordMetadata[] = await producer.send({
          topic: "topic-name-1",
          messages: [
            {
              partition: 42,
    ...
    describe("failed send", () => {
      beforeEach(async () => {
        patchProducerSend((): Promise<KafkaJS.RecordMetadata[]> => {
    ...
    describe("producer hook successful", () => {
      beforeEach(async () => {
        patchProducerSend(async (): Promise<KafkaJS.RecordMetadata[]> => []);

        const config: ConfluentKafkaInstrumentationConfig = {
  ...
  describe("consumer", () => {
    ...
    const createKafkaMessage = ({
      ...
      key = "message-key",
      tombstone = false,
    }: CreateMessageParams): KafkaJS.KafkaMessage => {
      return {
    ...
    const createEachBatchPayload = (): KafkaJS.EachBatchPayload => {
      return {
        batch: {
          topic: "topic-name-1",
          partition: 1234,
    ...
    describe("successful eachMessage", () => {
      ...
      it("consume eachMessage create span with expected attributes", async () => {
        consumer.run({
          eachMessage: async (
            _payload: KafkaJS.EachMessagePayload
          ): Promise<void> => {},
        });
      ...
      it("consumer eachMessage with non promise return value", async () => {
        consumer.run({
          ...
          eachMessage: async (_payload: KafkaJS.EachMessagePayload) => {},
        });
        const payload: KafkaJS.EachMessagePayload = createEachMessagePayload();
        await runConfig?.eachMessage!(payload);
    ...
    describe("eachMessage throws", () => {
      ...
      it("Error message written in the span status", async () => {
        ...
        consumer.run({
          eachMessage: async (_payload: KafkaJS.EachMessagePayload): Promise<void> => {
            throw errorToThrow;
          },
        });
    ...
    describe("successful eachBatch", () => {
      ...
      it("consume eachBatch create span with expected attributes", async () => {
        consumer.run({
          eachBatch: async (_payload: KafkaJS.EachBatchPayload): Promise<void> => {},
        });
        const payload: KafkaJS.EachBatchPayload = createEachBatchPayload();
      ...
      it("consumer eachBatch with non promise return value", async () => {
        consumer.run({
          ...
          // is not typescript valid, but it might (and is) implemented in real life (nestjs)
          // and does not break the library.
          eachBatch: async (_payload: KafkaJS.EachBatchPayload) => {
            return;
          },
        });
        const payload: KafkaJS.EachBatchPayload = createEachBatchPayload();
  ...
  describe("context propagation", () => {
    beforeEach(() => {
      ...
      instrumentation.enable();
      producer = kafka.producer();
      consumer = kafka.consumer({ kafkaJS: { groupId: "testing-group-id" } });
    });
    ...
    it("context injected in producer is extracted in consumer", async () => {
      ...
      assert.strictEqual(messagesSent.length, 1);
      const consumerPayload: KafkaJS.EachMessagePayload = {
        topic: "topic-name-1",
        partition: 0,
        message: {
          key: Buffer.alloc(0),
    ...
    it("context injected in producer is extracted as links in batch consumer", async () => {
      ...
      assert.strictEqual(messagesSent.length, 1);
      const consumerPayload: KafkaJS.EachBatchPayload = {
        batch: {
          topic: "topic-name-1",
          partition: 0,
          highWatermark: "1234",
